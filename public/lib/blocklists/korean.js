...
export const koreanJokeNames = [
  ...
  // ── Apellidos-broma: apellido KR común + vulgaridad concatenada (RR)
  ['kimssibal',          '"Kim Ssibal" → 씨발 vulgar genérico'],
  ['parksibal',          '"Park Sibal" → sibal vulgar'],
  ['leesibal',           '"Lee Sibal" → sibal vulgar'],
  ['choisibalnom',       '"Choi Sibalnom" → cabrón de mierda'],
  ['jungsibalsekki',     '"Jung Sibalsekki" → hijo de puta vulgar'],
  ['kimjiral',           '"Kim Jiral" → 지랄 vulgar'],
  ['parkjiral',          '"Park Jiral" → jiral vulgar'],
  ['leegaeseki',         '"Lee Gaeseki" → 개새끼 hijo de perro'],
  ['hangaesaekki',       '"Han Gaesaekki" → gaesaekki vulgar'],
  ['jeongaeseki',        '"Jeon Gaeseki" → gaeseki vulgar'],
  ['kimbyungshin',       '"Kim Byungshin" → 병신 slur capacitista'],
  ['shinbyungsin',       '"Shin Byungsin" → byungsin vulgar'],
  ['ohbyungsin',         '"Oh Byungsin" → byungsin capacitista'],
  ['kangjonnashibal',    '"Kang Jonna Shibal" → vulgar intensificado'],
  ['yoonjotgateun',      '"Yoon Jotgateun" → 좆같은 vulgar'],
  ['baekjotgachi',       '"Baek Jotgachi" → como una polla'],
  ['seojotka',           '"Seo Jotka" → vulgar pene+marca'],
  ['hwangboji',          '"Hwang Boji" → 보지 vagina vulgar'],
  ['kimbobbal',          '"Kim Bobbal" → vulgar sexual'],
  ['leechangnyeo',       '"Lee Changnyeo" → 창녀 puta'],
  ['parkmichinnom',      '"Park Michinnom" → cabrón loco'],
  ['kimmichinyeon',      '"Kim Michinyeon" → zorra loca'],
  ['choiddorai',         '"Choi Ddorai" → 또라이 loco vulgar'],
  ['shimttorai',         '"Shim Ttorai" → ttorai loco'],
  ['hongdeungsin',       '"Hong Deungsin" → 등신 idiota'],
  ['minsseuregi',        '"Min Sseuregi" → 쓰레기 basura'],
  ['ryuyangachi',        '"Ryu Yangachi" → vándalo despectivo'],
  ['kimjjokbari',        '"Kim Jjokbari" → slur anti-japonés'],
  ['parkjjangkkae',      '"Park Jjangkkae" → slur anti-chino'],
  ['nojucheist',         '"No Jucheist" → apología juche norcoreano'],
  ['hajongildaewangmanse','"Ha Jong-il Daewang Manse" → chant pro-Kim Jong-il'],
];
